...
mtype:Performance10_dispatch = {ALU1_turn, ALU2_turn, ALU3_turn, ALU4_turn, ALU5_turn, ALU6_turn, ALU7_turn, ALU8_turn, ALU9_turn, ALU10_turn, DONE_turn_Performance10};
proctype Performance5(chan EI_REQ, EO_CNF, VO_RES, alpha, beta) {
	bit ExistsInputEvent = 0;
	mtype:Performance10_dispatch dispatch_state = ALU1_turn;
	bit omega;
   ...
   dispatch: 
   	...
   	:: atomic { dispatch_state == ALU10_turn -> 
   		ALU10_alpha!true;
   		ALU10_beta?true;
   		dispatch_state = DONE_turn_Performance10;
   	}
   	:: dispatch_state == DONE_turn_Performance10 -> skip;
   	fi
   ...
   read_component_event_outputs:
   	atomic {
   		omega = empty(ALU1_EO_CNF) && empty(ALU2_EO_CNF) && empty(ALU3_EO_CNF) && empty(ALU4_EO_CNF) && empty(ALU5_EO_CNF) && empty(ALU6_EO_CNF) && empty(ALU7_EO_CNF) && empty(ALU8_EO_CNF) && empty(ALU9_EO_CNF) && empty(ALU10_EO_CNF);
   		...
   		:: nempty(ALU10_EO_CNF) -> 
   			...
   			reset(VO_RES);
   			EO_CNF!true;
   			VO_RES!buf_ALU10_RES;

   		:: (omega && dispatch_state == DONE_turn_Performance10) -> goto done;
   		:: (omega && dispatch_state != DONE_turn_Performance10) -> goto dispatch; 
   		fi
   	}
   	goto read_component_event_outputs;
