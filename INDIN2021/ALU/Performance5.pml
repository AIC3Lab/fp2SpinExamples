...
mtype:Performance5_dispatch = {ALU1_turn, ALU2_turn, ALU3_turn, ALU4_turn, ALU5_turn, DONE_turn_Performance5};
proctype Performance5(chan EI_REQ, EO_CNF, VO_RES, alpha, beta) {
	bit ExistsInputEvent = 0;
	mtype:Performance5_dispatch dispatch_state = ALU1_turn;
	bit omega;
   ...
   dispatch: 
   	...
   	:: atomic { dispatch_state == ALU5_turn -> 
   		ALU5_alpha!true;
   		ALU5_beta?true;
   		dispatch_state = DONE_turn_Performance5;
   	}
   	:: dispatch_state == DONE_turn_Performance5 -> skip;
   	fi
   ...
   read_component_event_outputs:
   	atomic {
   		omega = empty(ALU1_EO_CNF) && empty(ALU2_EO_CNF) && empty(ALU3_EO_CNF) && empty(ALU4_EO_CNF) && empty(ALU5_EO_CNF);
   		...
   		:: (omega && dispatch_state == DONE_turn_Performance5) -> goto done;
   		:: (omega && dispatch_state != DONE_turn_Performance5) -> goto dispatch; 
   		fi
   	}
   	goto read_component_event_outputs;
